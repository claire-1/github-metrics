...
public class CommentProcessorTest {
    ...
    public void givenDatabaseShouldBeAbleToRunClassifier() throws Exception {
        ...
        List<GHIssueComment> commentsForFirstIssue = IssueUtils.getComments(secondIssue);
        Date sqlDate = IssueUtils.getSqlDate(secondIssue);
        processorDB.putCommentsInDB(secondIssue.getId(), sqlDate, commentsForFirstIssue);
        ...
        processorDB.putClassificationInDB(secondIssue.getId(), sqlDate, classification);
        assertEquals("resolved", classification);
        assertEquals("resolved", processorDB.getDataFromDatabase(" select * from classifierResults"));
    }
